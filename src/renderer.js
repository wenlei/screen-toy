...
(function () {
  ...
  var flowerPhase = 0;
  var flowerFrameIdx = 0;
  var flowerFrameTimer = 0;
  var FLOWER_FRAME_MS = 100;
  ...
  // ── Animation dialogue (在动画进程中按百分比触发文字对话) ──────────────────
  ...
  // track which dialogue points have already fired for each active animation
  var animDialogueFired = {};

  function checkAnimDialogue(animName, currentFrame, totalFrames) {
    var dialogues = ANIM_DIALOGUES[animName] || [];
    if (totalFrames <= 0) return;
    var percent = Math.floor((currentFrame / totalFrames) * 100);
    var key = animName + '_p';
    dialogues.forEach(function (d) {
      if (percent >= d.p && !animDialogueFired[key + d.p]) {
        animDialogueFired[key + d.p] = true;
        if (window.screenToy) {
          window.screenToy.showBubble({ text: d.text, wait: 2000 }, true);
        }
      }
    });
  }
  ...
  if (window.screenToy) {
    ...
    window.screenToy.onTriggerAnimation(function (name) {
      // Suppress during active bubbles (don't check bubbleQueue — it may stay non-empty after animation ends)
      if (behavior.anim._bubbleText || (behavior.anim._bubbleQueue && behavior.anim._bubbleQueue.length > 0)) return;

      if (name === 'twist' && twistPhase === 0 && hulaPhase === 0 && !sneezeActive) {
        twistPhase = 1;
        twistFrameIdx = 0;
        twistFrameTimer = 0;
        animDialogueFired = {};  // Reset dialogue tracking
  ...
  function loop(timestamp) {
    try {
      ...
      if (enterFrameIdx >= 0) {
        ...
        if (enterFrameIdx >= ETOTAL) {
          ...
          if (window.EnterSprite) {
            ...
            var sy = Math.round((canvas.height - sh) / 2 + canvas.height * 0.15);
      ...
      if (sneezeActive) {
        ...
        if (sneezeActive) checkAnimDialogue('sneeze', sneezeFrameIdx, STOTAL);
      }
      if (okActive) {
      ...
      if (bigNoseActive) {
        ...
        if (bigNoseActive) checkAnimDialogue('bignose', bigNoseFrameIdx, BNTOTAL);
      }
      // ───────────────────────────────────────────────────────────────────────
      ...
      if (flowerPhase > 0) {
        ...
        var curFlowerSprite = flowerPhase === 1 ? window.Flower1Sprite : window.Flower2Sprite;
        var FLTOTAL = curFlowerSprite ? curFlowerSprite.FRAME_COUNT : 24;
        while (flowerFrameTimer >= FLOWER_FRAME_MS && flowerFrameIdx < FLTOTAL) {
          flowerFrameTimer -= FLOWER_FRAME_MS;
          flowerFrameIdx++;
        }
        if (flowerFrameIdx >= FLTOTAL) {
          if (flowerPhase === 1) {
            flowerPhase = 2;
            flowerFrameIdx = 0;
            flowerFrameTimer = 0;
        ...
        if (flowerPhase > 0) checkAnimDialogue('flower', (flowerPhase - 1) * 24 + flowerFrameIdx, 48);
      }
      // ───────────────────────────────────────────────────────────────────────
      ...
      if (applePhase > 0) {
        ...
        checkAnimDialogue('apple', (applePhase - 1) * 24 + appleFrameIdx, 48);
      }
      // ───────────────────────────────────────────────────────────────────────
      ...
      if (meltActive) {
        ...
        if (meltActive) checkAnimDialogue('melt', meltFrameIdx, MTOTAL);
      }
      // ───────────────────────────────────────────────────────────────────────
      ...
      if (freezePhase > 0) {
        ...
        if (freezePhase > 0) checkAnimDialogue('freeze', (freezePhase - 1) * 24 + freezeFrameIdx, 48);
      }
      // ───────────────────────────────────────────────────────────────────────
      ...
      if (hulaPhase === 0 && !sneezeActive && !meltActive && freezePhase === 0 && !sunGameActive && applePhase === 0 && twistPhase === 0 && !bigNoseActive && flowerPhase === 0) {
      ...
      var animRunning = twistPhase > 0 || hulaPhase > 0 || sneezeActive || meltActive || freezePhase > 0 || bigNoseActive || applePhase > 0 || sunGameActive || flowerPhase > 0;
      if (!animRunning) {
      ...
      if (hulaPhase > 0) {
        ...
        if (curFlowerSp) {
          var flFrameToShow = Math.min(flowerFrameIdx, curFlowerSp.FRAME_COUNT - 1);
          var flDW = Math.round(dW * 1.38);
          var flDH = Math.round(dH * 1.38);
          var flOx = Math.round(canvas.width / 2 - flDW / 2);
          var flOy = Math.round(canvas.height / 2 - flDH / 2);
          curFlowerSp.drawDirect(ctx, flFrameToShow, flOx, flOy, flDW, flDH);
        }
      ...
      console.error('[loop] exception:', e, e && e.stack);
    }
    requestAnimationFrame(loop);
  }
